...
export class stringTemplateOutletDirective implements OnChanges {
  ...
  private shouldRecreateView(changes: SimpleChanges): boolean {
    ...
    if (stringTemplateOutlet) {
      if (stringTemplateOutlet.firstChange) {
        ...
        const previousOutletType = this.getType(
          stringTemplateOutlet.previousValue
        );
        const currentOutletType = this.getType(
          stringTemplateOutlet.currentValue
        );
        shouldOutletRecreate = !(
          previousOutletType === "string" && currentOutletType === "string"
        );
      }
    }
  ...
  private updateExistingContext(ctx: any): void {
    for (const propName of Object.keys(ctx)) {
      // tslint:disable-next-line:no-any
      (this.inputViewRef!.context as any)[
        propName
      ] = this.stringTemplateOutletContext[propName];
    }
  }
