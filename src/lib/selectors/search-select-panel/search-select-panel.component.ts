...
import { debounce, map, publishReplay, refCount } from "rxjs/operators";
...
export class SearchSelectPanelComponent implements OnInit, OnDestroy {
  ...
  @Input()
  public localFilter:
    | ((items: IListItem[], search: string) => IListItem[])
    | "default" = "default";
  @Input()
  ...
  public ngOnInit() {
    ...
    const sharedSearchResult$ = this.searchResult$.pipe(
      takeUntilDestroy(this),
      publishReplay(1),
      refCount()
    );

    sharedSearchResult$.subscribe(
      searchResult => (this.searchResultItems = searchResult.items)
    );

    this.filteredSearchResult$ = combineLatest(
      sharedSearchResult$,
      this.innerSearch$
    ).pipe(
      map(([searchResult, search]) => {
        const localFilter =
          this.localFilter === "default"
            ? defaultFilter
    ...
    this.displayedSelectedItems$ = combineLatest(
      sharedSearchResult$,
    ...
    this.search$.pipe(takeUntilDestroy(this)).subscribe(search => {
    ...
    this.searchInput$
      .pipe(
        debounce(() =>
          this.searchDebounceTimeMs != null
            ? timer(this.searchDebounceTimeMs)
            : EMPTY
        )
      )
      .subscribe(newSearch => {
...
function defaultFilter(items: IListItem[], search: string): IListItem[] {
  ...
  return items.filter(
    item => (item.text || "").toLowerCase().indexOf(search) !== -1
  );
}
